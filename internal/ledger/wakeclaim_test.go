...
// §4-14/15：同 seq 并发只允许一个拿到；租约过期后允许第二者接管。
...
// §4-16：非持有者收尾失败且不改行；持有者收尾后该事件永不再被认领。
func TestCompleteWakeOnlyByHolder(t *testing.T) {
	...
	if err := s.CompleteWake(seq, "mbp"); !errors.Is(err, ErrCASConflict) {
		t.Fatalf("非持有者收尾应 ErrCASConflict，得 %v", err)
	}
	if _, err := s.ClaimWake(seq, "B1", "mbp", time.Minute); err != nil {
		t.Fatalf("非持有者收尾失败不应释放认领: %v", err)
	}
	if err := s.CompleteWake(seq, "linux-01"); err != nil {
		t.Fatalf("持有者收尾: %v", err)
	}
...
func TestCursorWatermarkStopsBeforeInFlightClaims(t *testing.T) {
	...
	if err := s.CompleteWake(12, "linux-01"); err != nil {
		t.Fatal(err)
	}
	if err := s.CompleteWake(18, "linux-01"); err != nil {
		t.Fatal(err)
	}
	...
	if w != 14 {
		t.Fatalf("在飞 15 应把水位挡在 14，得 %d", w)
	}
	// 在飞清单当前只有 15。
	...
	if err := s.CompleteWake(15, "linux-01"); err != nil {
		t.Fatal(err)
	}
	if w, err := s.CursorWatermark(10, 20); err != nil || w != 20 {
