package agentd

import (
	"testing"

	"github.com/Xsxdot/handoff/internal/workspace"
)

func TestID8MatchesWorkspaceCanonical(t *testing.T) {
	samples := []string{
		"abcdefgh-rest",
		"abcdefgh",
		"short",
		"137a7dc9-df89-4c1c-891e-ebe106c68b37",
	}
	for _, s := range samples {
		if got, want := id8(s), workspace.ID8(s); got != want {
			t.Fatalf("id8(%q) = %q, workspace.ID8 = %q", s, got, want)
		}
		if got, want := taskBranch(s), workspace.TaskBranch(s); got != want {
			t.Fatalf("taskBranch(%q) = %q, workspace.TaskBranch = %q", s, got, want)
		}
	}
}

func TestBaseCommitReMatchesWorkspaceIsCommitSHA(t *testing.T) {
	samples := []string{
		"",
		"482aab1f9e12a3b4c5d6e7f8a9b0c1d2e3f4a5b6",
		"482aab1",
		"482AAB1F9E12A3B4C5D6E7F8A9B0C1D2E3F4A5B6",
		"origin/main",
	}
	for _, s := range samples {
		if got, want := baseCommitRe.MatchString(s), workspace.IsCommitSHA(s); got != want {
			t.Fatalf("baseCommitRe(%q)=%v, workspace.IsCommitSHA=%v", s, got, want)
		}
	}
}
