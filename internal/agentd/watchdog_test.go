...
func TestWatchdogFiresOnceOnStall(t *testing.T) {
	...
	go runWatchdog(ctx, st, hub, time.Nanosecond, 10*time.Millisecond, 0, 0, func(string, string) error { return nil }, discardLogger())
...
func TestWatchdogIgnoresFreshAndTerminal(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	go runWatchdog(ctx, st, hub, time.Hour, 10*time.Millisecond, 0, 0, func(string, string) error { return nil }, discardLogger())
	time.Sleep(30 * time.Millisecond)
	cancel()
