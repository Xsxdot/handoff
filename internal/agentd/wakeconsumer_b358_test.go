...
func TestB3583ReplyToReboundOldSeatDoesNotWake(t *testing.T) {
	...
	if err := env.ledger.RebindSeat(cardID, newIdentity, proto.SeatSourceCoordinate, oldSeat.DriverSession, ledger.SeatBearing{Carrier: "coord-carrier", Machine: "local"}); err != nil {
		t.Fatalf("换绑: %v", err)
	}
...
func TestB3583StructureEventsNeverWake(t *testing.T) {
	...
	if err := env.ledger.BindSeat(freshCard, "cli:opencode#seat-bound", proto.SeatSourceCoordinate, ledger.SeatBearing{Carrier: "coord-carrier", Machine: "local"}); err != nil {
		t.Fatalf("坐下: %v", err)
	}
	if err := env.ledger.RebindSeat(freshCard, "cli:opencode#seat-rebound", proto.SeatSourceCoordinate, "cli:opencode#seat-bound", ledger.SeatBearing{Carrier: "coord-carrier", Machine: "local"}); err != nil {
		t.Fatalf("换绑: %v", err)
	}
