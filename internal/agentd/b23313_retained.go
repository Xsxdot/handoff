...
// —— gateway 未导出助手的导出入口（P3；单一实现留在原声明处）——

// CheckProcHeadroom 见 admission.go#checkProcHeadroom。
