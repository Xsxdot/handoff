...
func TestClassify(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			ms:   Machine{Name: "devbox", Err: errors.New("dial tcp: connection refused")},
			want: VerdictUnreachable,
		},
		{
			name: "本机 agentd 未运行：不是失败，敲命令的人知道要不要起回来",
			ms:   Machine{Name: "本机", Local: true, Bin: "v0.1.0", Err: client.ErrStatusUnsupported},
			want: VerdictAgentdDown,
		},
		{
			name: "本机 agentd 未运行但二进制已最新：没事可做，不该重下重换",
			ms:   Machine{Name: "本机", Local: true, Bin: latest, Err: errors.New("connection refused")},
			want: VerdictLatest,
		},
		{
			name: "远端过旧未上报平台：排在托管判定之前",
			ms:   Machine{Name: "devbox", Agentd: "v0.1.0", Platform: ""},
			want: VerdictTooOld,
		},
		{
			name: "远端过旧且未上报托管：仍报过旧，不得报非托管（B64 原始症状）",
			ms:   Machine{Name: "devbox", Agentd: "v0.1.0", Platform: "", Managed: nil},
			want: VerdictTooOld,
		},
		{
			name: "远端非托管但已是最新：没事可做，不该催人装 service",
			ms: Machine{Name: "devbox", Agentd: latest,
		...
		},
		{
			...
			ms: Machine{Name: "devbox", Agentd: "v0.1.1-rc", Platform: "linux/amd64",
		...
		},
		{
			...
			ms: Machine{Name: "本机", Local: true, Bin: latest, Agentd: "v0.1.0",
