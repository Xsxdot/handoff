...
func (a *Adapter) Name() string { return executor.HarnessCodex }

// Report 返回本家能力矩阵。查询面与 BaselineReport 对齐；「支持」不等于已接线。
func (a *Adapter) Report() executor.CapabilityReport {
	rep, err := executor.BaselineReport(executor.HarnessCodex)
	if err != nil {
		if a != nil && a.log != nil {
			a.log.Error("BaselineReport 失败，返回空能力报告", "harness", executor.HarnessCodex, "cause", err)
		}
		return executor.CapabilityReport{Harness: executor.HarnessCodex}
	}
	return rep
}
