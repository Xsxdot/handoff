// fallback_verdict_test.go —— codex finishTurn 兜底分支的裁决测试（B74）。
//
// 兜底嵌在 finishTurn 的 switch kind default 分支里，测试经 FinishTurnForTest
// 直接驱动：事实源是真实临时 git 仓库（GitTurnStatus 真跑 git 子进程，桩不了），
// hasNew=true 时在回合起点后补一次提交。askedViaTool 抑制与零文本守卫两道闸
// 各有一条回归护栏——它们在本 plan 里一行不动。
// 会话标识是 threadID 而非 sessionID，接线时最容易漏，单独断言守住。
package codex
...
//
// 返回：adapter、runState（repoPath=真仓库，startCommit=回合起点，threadID 已填）、
...
func newAdapterWithFakeGit(t *testing.T, branch string, hasNew bool) (*Adapter, *runState, string) {
	...
	a, r := NewAdapterWithRunForTest("T-1")
	r.repoPath = repo
	r.startCommit = startCommit
	r.threadID = "thread-1"
	return a, r, head
}

// finishTurn 走一次 codex 的回合收尾分类（status="" 表示非失败非中断的正常收尾）。
func finishTurn(a *Adapter, r *runState, text string) {
	FinishTurnForTest(a, r, "", "", text)
}
...
func TestNoTrailerWithNewCommitDoesNotDeclareCompletion(t *testing.T) {
	...
	finishTurn(a, r, "我把 Task 5 的三个方案列出来了，你选哪个？")
...
func TestNoTrailerAskedViaToolStillSuppresses(t *testing.T) {
	...
	r.noteAskedViaTool()
	finishTurn(a, r, "已调用一次提问工具；本回合结束。")
