...
// workspace.PrepareWorkspace 成功之后、任务记录落库之前失败时，补偿清理已建的 managed
// worktree（P2-2）：否则该 worktree 无任务记录持有，done 永不清理成为孤儿。
func TestDispatchFailedAfterWorkspaceCleansManagedWorktree(t *testing.T) {
	repo := initTestRepo(t)
	// DataDir 下预置一个名为 tasks 的**文件**：workspace.PrepareWorkspace 的 worktrees
...
// compensateFixture 造一个「workspace.PrepareWorkspace 之后必失败」的 Manager：
