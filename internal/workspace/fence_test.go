// fence_test.go —— RunCmd 的 fork 失败归因与错误语义测试（B233.15 从 agentd 迁入）。
//
// 职责：钉住「归因只改文案、不改错误语义」以及归因文案能到达调用者。
// 边界：进程余量准入闸的测试留在 internal/agentd（CheckProcHeadroom）。
package workspace
...
func TestForkFailureNoteReachesCaller(t *testing.T) {
	SetForkFailureNote(prochost.ExplainForkFailure)
	defer SetForkFailureNote(nil)
