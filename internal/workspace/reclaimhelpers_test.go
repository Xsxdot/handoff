// reclaimhelpers_test.go —— workspace 包内 worktree 巡检解析/判定测试（B233.15 从 agentd 迁入）。
//
// 职责：固定文本解析 + 真实工作树四态判定。
// 边界：回收编排（Reclaim/ReclaimList）的测试留在 internal/agentd。
package workspace
...
func TestFindEntryMatchesAcrossSymlink(t *testing.T) {
	...
	if _, ok := FindWorktree(entries, link); !ok {
		t.Fatalf("经符号链接给的 workdir 应能匹配到条目")
	}
	if _, ok := FindWorktree(entries, filepath.Join(real, "other")); ok {
		t.Fatalf("不同路径不该匹配")
	}
}
...
func TestClassifyModifiedIsDirty(t *testing.T) {
	...
	entries, _ := ListWorktrees(context.Background(), repo)
	state, dirty, _ := ClassifyWorktree(context.Background(), entries, wt)
	if state != proto.WorktreeDirty {
