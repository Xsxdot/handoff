...
func TestResolveBaseBranchAlwaysFetches(t *testing.T) {
	...
	got, err := resolveRemoteBaseBranch(context.Background(), clone, "origin", "main")
	if err != nil {
...
func TestResolveBaseBranchMissingBranch(t *testing.T) {
	_, clone := newOriginAndClone(t)
	_, err := resolveRemoteBaseBranch(context.Background(), clone, "origin", "no-such-branch")
	if err == nil {
		t.Fatalf("不存在的分支应报错")
	}
...
func TestResolveBaseBranchConcurrentFetchUsesRemoteRef(t *testing.T) {
	...
	for range 2 {
		go func() {
			sha, err := resolveRemoteBaseBranch(context.Background(), clone, "origin", "main")
...
func TestResolveBaselineAndBranchShareRepoFetchLock(t *testing.T) {
	...
	go func() {
		branch, err := resolveRemoteBaseBranch(context.Background(), clone, "origin", "main")
...
func TestResolveBaseBranchLockContentionHasIndependentSentinel(t *testing.T) {
	...
	_, err := resolveRemoteBaseBranch(context.Background(), t.TempDir(), "origin", "main")
