...
- **codex**: the executor machine has run `codex login`. Three things to know:
  ...
  - Permission model: headless `agy` reads `$HOME/.gemini/config/hooks.json`. handoff injects a task-local `HOME=<taskDir>/agyhome`, copies only the login token into it, and never writes the user's global config. The task HOME's `handoff-safety-gate` uses PreToolUse matcher `*` so every tool goes to the task `perm.sock` and Handoff's permission pipeline. `--dangerously-skip-permissions` skips native print-mode confirmation only; hook deny still vetoes. handoff does not write `always-proceed`. The gate is written only into the task HOME, not workspace `.agents/hooks.json` (which `--add-dir` would also load). Task HOME remains inside `taskDir`. By default `--sandbox` is not passed so task-private `TMPDIR`/`GOCACHE` stay writable.
