...
import { describeElement, logTermFocus, logTermHost, logTermInput, logTermKeepalive, logTermResize, logTermWheel, terminalDebugEnabled } from './terminalDebug'
...
export function TerminalTab({
  ...
  useEffect(() => {
    ...
    // 滚轮就丢了。host 一直在，不依赖 xterm 的协议绑定。
    ...
    const handleAltWheel = (ev: WheelEvent): boolean => {
      ...
      const seq = altBufferWheelSgr(deltaY, cellH, wheelRemainder, col, row)
      if (seq !== '') {
        // host 捕获阶段和 xterm 自己的 wheel 可能看到同一事件；只发一次。
        if (!wheelOnce.has(ev)) {
          wheelOnce.add(ev)
