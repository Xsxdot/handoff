      ...
      {dialog !== null && draft !== null && <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/30 p-4" role="dialog" aria-modal="true" aria-labelledby="scheduling-dialog-title">
        <form className={`w-full space-y-4 rounded-lg border bg-background p-5 shadow-lg ${dialog.kind === 'carrier' ? 'max-w-[480px]' : 'max-w-[440px]'}`} onSubmit={(event) => { event.preventDefault(); void save() }}>
          ...
          {dialog.kind === 'carrier' ? <div className="grid gap-x-3 gap-y-2.5 sm:grid-cols-2">
            <div><label htmlFor="sched-carrier-name" className={FIELD_LABEL}>载体名（唯一，登记后不可改）</label><input id="sched-carrier-name" aria-label="载体名" className={INPUT} readOnly={dialog.value !== null} value={draft.name} onChange={(event) => updateDraft({ name: event.target.value })} /></div>
            ...
            <div className="sm:col-span-2"><label htmlFor="sched-carrier-home" className={FIELD_LABEL}>HOME 档案（隔离 HOME 路径；协调者 = 全套，执行者 = 干净会话）</label><input id="sched-carrier-home" aria-label="HOME 档案" className={INPUT} value={(draft as CarrierDraft).home_dir} onChange={(event) => updateDraft({ home_dir: event.target.value })} /></div>
            ...
            <fieldset className="sm:col-span-2"><legend className={FIELD_LABEL}>成员载体（按勾选顺序解析：第一个健康且有空的载体领活）</legend><div className="flex max-h-[140px] flex-col gap-1 overflow-y-auto rounded-md border p-2 text-xs">{snapshot.carriers.map((carrier) => <label key={carrier.name} className="flex items-center gap-1.5"><input type="checkbox" checked={(draft as SquadDraft).members.includes(carrier.name)} onChange={(event) => { const members = (draft as SquadDraft).members; updateDraft({ members: event.target.checked ? [...members, carrier.name] : members.filter((member) => member !== carrier.name) }) }} /> <span className="font-mono">{carrier.name}</span> <span className="text-muted-foreground">{carrier.machine} · {carrier.cli}</span></label>)}{snapshot.carriers.length === 0 && <p className="text-muted-foreground">尚未登记载体，先到上方登记一个。</p>}</div></fieldset>
            <div><label htmlFor="sched-squad-conc" className={FIELD_LABEL}>并发上限（政策位；0 / 留空 = 不限）</label><input id="sched-squad-conc" aria-label="并发上限" className={INPUT} type="number" min="0" value={(draft as SquadDraft).maxConcurrencyText} onChange={(event) => updateDraft({ maxConcurrencyText: event.target.value })} /></div>
