...
//   - 配对开发机与 Env 文件不渲染——那两项连形态都还没定。「重启 agent」
//     与「打开终端」曾以「可点、点了说尚未实现」的形态占位，已删除：占位本身
//     也是一种承诺，而这两件事都还没有后端接口，留着只是让用户反复点。
//
// 写操作：
    ...
    >
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1 text-sm">
          {/* 令牌用密码型输入框：令牌随请求体发送并随开发机配置落盘到 agentd
              配置文件（增删机器的写操作本来就要改配置）；后端不回显它，也不会
              把它写进任何日志。本输入框只在提交时作为请求体发走，不留本地持久化。 */}
