...
describe('eventPhrase', () => {
  ...
  it('后台审计噪声返回 null，由会话流过滤但不影响原始事件调试视图', () => {
    expect(eventPhrase('approver_decision')).toBeNull()
    expect(eventPhrase('permission_reuse')).toBeNull()
    expect(eventPhrase('progress')).toBeNull()
  })
  it('未知类型原样透出，不吞', () => {
