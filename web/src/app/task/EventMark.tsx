...
//     EventsPanel 管实时、全局工单弹层管裁决。审批不可逆且要当真，
//     给它开第二个入口、还开在一条摘要旁边，风险和收益不成比例
//   - 不显示 payload 全文。全文只在工单面板（EventsPanel 头注释的既有纪律）
...
// 只有这两类才把人指向工单面板：completed / failed / delivery_failed / stalled
// 都没有可裁决物（它们的出口分别是审核、重新派发、resume、attach 判活），
// 在它们旁边写「裁决入口在左栏底部的工单面板」是纯噪音，还会让人去工单面板扑空。
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 rounded-md border border-amber-500/40 bg-amber-500/5 px-2.5 py-1.5 text-xs">
      ...
      <span className="text-[11px] text-muted-foreground">
        {formatFull(ts)}
        {ADJUDICABLE.has(event) && ' · 裁决入口在左栏底部的工单面板'}
      </span>
    </div>
  )
}
