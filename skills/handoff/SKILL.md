...
| 谁 | 推什么 |
|---|---|
| **代码自动** | 派发即认领（待办→进行中，CAS）；一切失败出口打「等人」标记；合并节点成功后推「待合并」 |
| **你（主会话）** | 中间的推进：task 完成→待审阅、裁决 pass→下一态、集成分支合完→已完成 |
| **人** | 答裁决、批工单、合 main、主线卡推「已完成」 |

一期你就是那台发动机。三期规则引擎接手「按按钮」的活，调的是同一个节点执行器。
...
修复回合计数、节点走到第几轮这类推进状态**一律从事件流推导**，不存会话记忆——
...
| 「审阅没过，我再 continue 一轮」 | 节点自己会 continue，3 轮封顶。手工绕开封顶就是绕开防死循环的安全阀。 |
| 「验过了，accept 一下，证据就不写了」 | 已验必须带证据，命令会直接拒。这是取证文化不是输入校验。 |
| 「合并节点跑完就合进 main 了吧」 | 主线永远人工。它只会把卡推到「待合并」等你。 |
