...
- **agy**（Antigravity CLI）：执行机安装 `agy` 并完成登录（`agy -p "hi"` 能出结果）。
  - 权限模型：通过工作区 `.agents/hooks.json` 的 `PreToolUse` 钩子动态连接到任务的 `perm.sock`，拦截 `run_command`、文件写/编辑与联网工具进入 Handoff 审批链；启动参数默认开启 `--sandbox` 提供终端限制，未挂钩工具受 agy 原生策略与本地配置保护。
