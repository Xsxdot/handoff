...
- **载体 HOME 含 `~` 时子进程在仓库下建出 `./~/` 垃圾目录。** 载体登记串是跨机可认的用户可见形态（含 `~`）；`agentd` 与 `hostapi` 进子进程 `HOME` 前未展开，子进程把 `$HOME` 系文件相对 cwd 落盘（2026-09-04 实测 2.2G）。现在两处都在注入环境前展开为绝对路径，展开失败保持原行为不阻断派发；`.gitignore` 加 `/~/` 永不入库。
- **工作项页空白处滚动仍带动整页（B318 没修完）。** B318 的 `min-h-0` 只钉住了常驻房间栏的高度。`/cards` 上工作台仍常驻在 `main` 里，flex 默认 `min-width:auto` 按终端固有宽把 `main` 撑到两万像素，房间栏被推到视口外，横滑空白处整页（含左栏）跟着走。`main` 补 `min-w-0 overflow-hidden`，壳和常驻栏切断溢出。
...
- **左栏任务列表顺序固定：按创建时间降序，打开任务不再洗牌。** 任务组、「已结束」与「未归属」统一显式按 created_at 降序——跨机聚合响应的行序本就不可信，此前打开一个任务整列都会重排；已打开的任务在任务流原位呈现已打开态，不再置顶。刚跑完的任务留在任务列表 30 分钟再沉「已结束」，tab 还开着就一直留。
- 派发任务行显示分支名：无名派发拿提示词前 20 字当名字，所有 charter 卡的行名千篇一律认不出谁是谁；现在识别出「名字是计划摘要的回声」时改显分支（`cards/Bxxx`）。行尾圆点按行类表达状态：任务行 = 任务状态（运行绿 / 等工单与待审琥珀 / 终态灰 / 失败红），终端行 = 连接状态（绿连红断），文件行 = 文件状态（干净绿 / 改动琥珀 / 冲突红 / 已删灰）。已打开行（终端 / 文件 / 任务）悬停时右侧滑出 × 快速关闭 tab——带会话的终端与未保存草稿仍走原确认弹层，与窗格 × 同一条规则。
...
- **工作项页空白处滚轮不再带动整页。** `/cards` 把房间会话挂成右侧常驻栏后，缺 `min-h-0`，会话列表会把页面撑得比窗口高，左栏和顶栏一起被滚走。中央容器和常驻栏补上这条高度约束，列表改在栏内滚。
