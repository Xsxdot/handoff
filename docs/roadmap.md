...
- 镜像 watermark 逐 seq 连续性对账（补洞重拉，根治「实时流错过审计类事件」整族）——来源：B380 spec §3.3。
- `tickets_voided` 发布语义修订（与 completed/failed 同刻产生，是否仍需镜像可见性待判）——来源：B380 spec §3.3。
...
- **三期承接（B156.3 的输入）**：规则引擎自动拉起协调者会话（本期拉起一律由用户执行）；
  兼任/分派的定性策略（本期机制双轨不定性）；限额检测与自动接手（本期只有手动换绑+
  重建四步）；答复/消息事件主动唤醒休眠会话（本期送达=回合末收+resume 后查账两条路）；
  叙事员轮再评估（本期弃选）。来源：`specs/2026-08-25-b156.2-collaboration-rooms-design.md`
  Out of Scope。
- **多人时代（触发条件：第二个人类成员共用账本）**：多人类成员与登录权限体系、线程、
  已读回执、在场状态、消息搜索。数据形状已预留（消息 author、房间成员集、按成员未读
  游标），届时是加数据不是改模型。来源：同上 spec 的 Out of Scope。
...
## 来自 B279 spec（2026-08-28，B156.3 前端控制台）——本期不做、后续要做

- **调度可视化扩展**：排队横带只呈现当前队列快照（`GET /api/queue`）；历史队列、
  清队时序、小队水位（政策位/物理位占用图）无前端面，需要时另开卡。
- **载体/小队登记的机器发现**：本期登记表单全手填；机器清单其实已在开发机聚合里
  （设置·开发机），自动预填候选是一次数值投影 + 表单联动，另开卡。
...
- **冻结绑定存量盘点**：B398 修的是「已有身份快照被路由归一改写」这一条路径；盘一遍还有多少卡的冻结绑定受过同类改写（含 B233.27「冻结派发：接收机必须是载体登记机」同族），值得在实现后用只读取数走一遍。来源：B398 spec §7 Out of Scope。

## 来自 B399 spec（2026-09-22，本期不做、后续要做）

- **协调者回合成本观测**：每轮唤醒的 token / 时长 / 动作数落成可读数，用于判断「30 分钟上界是否够」与后续调参（今天只能靠人说「光判断就用了五分钟」）。来源：B399 spec §7 Out of Scope。
- **唤醒失败期间账本不可用**：已拆成独立卡 **B401**（现象已证、机制未定位），不属 B399 范围。

## 来自 B398 验收 / B399 plan（2026-09-23，本期不做、后续要做）

- **接收端请求身份日志的指针打印**：`internal/agentd/handlers.go` 把 `frozen_target` 以 `*string` 指针作日志属性值，文本模式打印的是地址（生产 JSON 模式解引用正常）。B398 review 报 minor，协调者裁定留待后续。来源：B398 review（2026-09-23）。
- **B398 plan §6(c)/§12 测试清单未回写**：实现轮的变异暴露该清单声明失真（实现者补测但未同步 plan）；review 报 minor 并归协调者裁决。裁定：与其它文档欠账同批处理，不单独返工。来源：B398 review（2026-09-23）。
- **转交唤醒 HTTP 写超时 11m < 新上界 30m**：B399 把唤醒回合上界提到 30 分钟后，跨机「转交唤醒」的响应可能先超时，给发起机留**假失败标记**。plan §8 已记录并建议单开卡。来源：B399 plan（2026-09-23）。
- **裁决 trailer 围栏未顶行 ⇒ 裁决解析失败**：B399 implement 第 2 轮的 trailer 语义为 pass，但围栏未顶行（与正文粘连）导致解析失败、落 needs_human。解析侧应容忍行内围栏，或在纪律块里明确要求围栏顶行。来源：B399 implement 第 2 轮（2026-09-23，人工裁决记录在卡上）。
- **变异未还原即提交**：B399 上一轮（2580425c）把 `case errors.Is(ErrTurnTimeout)` 写成 `case false` 提交（变异残留），同轮还漏了 hostapi 改造致全仓 build 红；下一轮才补回。值得在实现纪律/收口自检里加一条「提交前跑变异的还原核对」。来源：B399 implement 第 2 轮（2026-09-23）。

## 来自 B400 spec / review（2026-09-23，本期不做、后续要做）

- **基线可见性**：`card show` 暴露 `base_branch` 与其解析来源（远端默认分支 / 显式声明 / 继承），排查误基线时不必再绕道 `effective_base_branch`。来源：B400 spec §7。
- **护栏 fail-open 残余**：项目仅登记在远端开发机（本机无位置）时，首派基线护栏探不到基线树、跳过检查（P1 甲，Warn 留痕）——「远端单机仓 + 默认线错」仍不被拦。来源：B400 plan §7 残余风险。
- **裸 `card dispatch`（无 `--step`）不经基线护栏**：调用方装配在 cmd 侧、探针未接线；该入口本就非卡驱动法定路径。来源：B400 plan §2 分流 / §7。
- **误基线纠正能力**：把卡的工作分支指到正确分支（含冻结点后的纠正窗口）归 B382「人工登记工作分支」机制。来源：B400 spec §7。
