...
## 来自 B370 spec（2026-09-14，镜像身份闸降级判定）

- **写入者版本偏斜的显性侦测**：消费点区分不了「旧构建写的缺 `node` 键」与「新构建写的空值」，B370 按同一形态（无从判定即放行）处理；要的是让偏斜可见，不是让判定更细。来源：B370 spec Out of Scope。
- **派发侧工作流身份巡检**：裸派发 / 不带 `--step` 的派发没有工作流身份是合法形态，B370 之后不再因此静默，但「谁在不带身份派发」仍无从发现。来源：B370 spec Out of Scope。

## 来自 B358.9 spec（2026-09-17，会话身份本质重做）
...
- **真实浏览器渲染未验**：plan §10.1 的「缺名横幅 / 新建表单只有标题 / 落款人名+端名 / `device` 缺失回退仅人名」只到 jsdom + 组件契约级（`agent-browser` 未安装，无法真机渲染核验）。工具就绪后补验，验收台账 §5.1 已记。
