...
- 2026-08-18 收口补修（grok 正向注册覆盖与 M1 注释）完成；正向测试在符号链接能力不可用时按理由跳过。Commit 范围：`HEAD^..HEAD`（本次收口提交）。

## Minor 记账

- M1：`cmd/agentd.go` 的注册表注释曾引用旧测试名；已改为 `TestAdapterRegistryHasAlwaysAvailableExecutors`（本次收口修复）。
