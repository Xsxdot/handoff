...
| Task 9 | 修复 1 | 验收测试首次立即检查目录，原始失败：`survive_test.go:73: Close 后会话目录应清掉: <nil>`；改为等待 ptyhost 异步收摊完成后再断言 | `internal/ptyhost/survive_test.go` |
| Task 9 | 1 | 双裁决通过：真实 hostproc 下 h1 detach、Scan live、h2 Adopt，backlog 必含 BEFORE、继续写入 AFTER、Close 清目录；正常用例与装饰性变异后均实跑，变异原始失败：`h2 backlog = ""，必须含客户端消失前的 BEFORE`；变异已撤销。ptyhost 全包测试与两平台构建通过 | `internal/ptyhost/survive_test.go` |
| Task 10 | 1 | 双裁决通过：机器级 CheckAdmission 仅扣除已登记且 PID/启动时刻双匹配的 ptyhost；普通同 uid、PID 复用与未知身份继续计数；凭据提供者读写加锁，任务级 footprint/RLIMIT_NPROC 未改。prochost、ptyhost、agentd 定向测试及两平台构建通过。全量 `go test ./...` 原始失败：`TestCursorRootFallsBackToCwdWhenHomeUnwritable`、`TestCursorRootErrorNamesBothPaths`、`TestLoadStripUpdateDoesNotBlockOnSaveFailure`、3 个 `TestPermServer...` 路径过长、`TestSyncAuthKeepsTaskCopyWhenWriteFails` 与 internal/client 相关失败 | `internal/prochost/{procenum,fence,ptyhost_pressure_test}.go`, `internal/ptyhost/client.go` |
| Task 10 | 终审修复 1 | 补真实 hostproc→Host.Adopt→CheckAdmission 覆盖：测试确认已登记会话的凭据确实让机器级计数减少 1；定向测试通过，准备并入 Task 10 提交 | `internal/ptyhost/client_test.go` |
| Follow-up | 双裁决通过 | 修正优雅关停接线：通用 cleanup 只取消 watchdog，保留显式 `ShutdownPtySessions` 入口；Trigger 回归测试确认已登记 PTY 会话仍在且未进入 Close 路径 | `cmd/agentd.go`, `internal/agentd/ptyreclaim.go`, `internal/agentd/ptyreclaim_test.go` |
