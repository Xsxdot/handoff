...
### 2.5 `Uninstall()`：先停再删，且 `/End` 是对的

**本节已被真机推翻重写**（08-18，见 §8）。原文说「不能依赖 `schtasks /End`」，
并让 `Uninstall` 先 `/Delete` 再自己按 pid 杀进程。两处都错：

- **`/End` 在本实现里是对的**。D8 那次 `/End` 只杀掉外层，是因为手搓任务套了
  `cmd.exe /c`，调度器跟踪的是 cmd、agentd 是它的孙进程。§2.2 第 4 条不套 cmd 之后，
  任务的动作进程直接就是 `handoff.exe`，`/End` 精确命中。
  而且这是**唯一**能精确命中的办法：agentd 与操作者正在敲的 `handoff` CLI
  同一个镜像名，按镜像名杀会把发出卸载命令的 CLI 自己一起杀掉
  （同一个坑在 §2.4 的 `Running` 判据上也踩过一次）。
- **顺序必须是先停再删**。先删任务，调度器就不再认识那个进程，agentd 会活下来
  变成「没人托管、也没人知道它还在」的孤儿。

`Uninstall` 的顺序：

1. `schtasks /End /TN <TaskName>` —— 停掉正在跑的 agentd。任务不在或本来就没跑时
   报错属正常，只记 Debug，**不中断卸载**（否则会停在「任务已删、XML 还在」的半途）。
2. `schtasks /Delete /TN <TaskName> /F` —— 摘掉托管。同样，报错只记 Debug。
...
5. **`Uninstall` 漏了「停」这一步**（§2.5）。任务与 XML 都删干净了，agentd 活过
   75 秒仍在，而报文是「agentd 不再被自动拉起」——读起来像已经停了。
   紧接着的 `install` 又因为它占着 DataDir 锁而失败，机器停在一个
   「没托管、装不上、还在跑」的状态。**只删不停不是少做一步，是把机器留在
   一个比卸载前更坏的状态里。**
