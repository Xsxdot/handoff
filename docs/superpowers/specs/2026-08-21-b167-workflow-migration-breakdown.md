...
# 契约拍板记录（协调者 + 用户，2026-08-21）

> **本节是决定，不是提案。** 上文所有标注「待拍板」的岔口按本节收口；
> 与上文提案冲突时**以本节为准**。Ticket 0 照本节落骨架 commit。

## 决定 ①：`MigrateCardWorkflow` → 方案 B（直接改签名，不留 wrapper）

**拍板**：不保留 deprecated wrapper。Ticket 0 一并更新仓内全部调用方
（实测 2 处：`internal/ledger/workflows_test.go`、`cmd/workflow.go`）与相关注释引用。
CLI 旧 `workflow migrate <id> --to <version>` 形状**一并删除**，只留新的跨流入口。

**理由**（推翻上文对方案 A 的倾向）：

1. 上文「外部包也失去源兼容」这条**不成立**——`internal/ledger` 是 internal 包，
   模块外根本无法 import，不存在外部调用方。
2. 上文自己写明方案 A 的 wrapper「仍能绕过『调用方显式写落点』和在飞门禁」。
   本需求的核心产出就是这两道门；留一条能绕过它们的旧路径，等于在新门旁边开一扇没锁的侧门。
3. 破坏面实测只有 2 处，成本远低于长期留一条未标记旧路径的风险。

## 决定 ②：目标版本 → 方案 α（可省略，0 = 迁移事务内取目标流最新版）

**拍板**：请求可省略 `version`；省略/0 时在迁移事务内解析目标流最新版本，
**并把解析出的版本号写进迁移事件与卡**（审计链必须能回答「当时迁到了哪一版」）。

**理由**：方案 β 防的是「填表时最新、提交时另一版」的竞态，触发条件是「有人正好在你
填表期间发布了新版工作流」——罕见；代价是每个调用方（Web/CLI）都要先读版本再处理过期
错误。α 的审计链完整，且日后可单向收紧成 β（加可选参数→改必填），反向难。

## 决定 ④：CLI 的在飞门禁 → **选项 3：从镜像事件流派生**（上文未列出的第三选项）

**拍板**：不采纳上文的选项 1（CLI 改走 HTTP）与选项 2（承认两通道语义不同）。
改为**在账本域派生在飞态**，两个通道读同一份事实。

**契约形状**（Ticket 0 落这个骨架）：

- 账本域新增派生查询，签名形如
  `func (s *Store) CardStepInFlight(cardID string) (bool, error)`
  ——按既有 `OpenTicketCounts`（`internal/ledger/taskstate.go`）的同一机制，
  单遍扫描 `EvTaskMirrored` 镜像事件回放任务生命周期。**沿用它，不要另造。**
- **在飞检查放在 Store 的迁移事务内**，不在 HTTP handler 里。两个入口（HTTP / CLI）
  因此自动共享同一门禁，无需 CLI 依赖 agentd 在跑。
- agentd 的 `cardStepFlight` map（`internal/agentd/cardstep.go`）**保留不动**：
  它守的是「同一进程内同一张卡同时只跑一个环节」的并发控制，与「这张卡当前是否有活在跑」
  是两个问题，可以并存。**但迁移门禁不读它。**

**理由**：

1. 选项 2 正是 2026-08-21 刚踩过的缺陷形状——CLI 能挂 `contract` 附件而 HTTP 400，
   两通道语义分裂害我们多跑一轮修复。明知故犯地再造一个同族缺陷不可接受。
2. 选项 1 让 CLI 从此依赖 agentd 在跑，丢掉它现在的独立可用性（`openLedger()` 直连账本），
   并把一个逻辑域问题拖进边界域（需要真机清单）。
3. `OpenTicketCounts` 已是**同一问题的既有解法与先例**，其注释写明设计原则：
   「镜像滞后即工单滞后——与实况同一显性化通道（MirrorHealth），**不另设真相源**」。
   agentd 那个进程内 map 正是第二个真相源；派生方案消除它，与既有架构同构。
4. 附带收益：进程内 map 在 agentd 重启后清空（代码注释里记为「刻意取舍」），
   派生方案没有这个盲区——重启后依然看得见还没收尾的环节。

**接受的取舍（不要在实现里"修"它）**：镜像滞后 → 在飞判定滞后。这与 `OpenTicketCounts`
是同一取舍，健康度经既有 MirrorHealth 通道显性化，**不另设真相源**。

## 对子卡划分的影响

- **B167-L（账本域）** 增加一项：`CardStepInFlight` 派生查询 + 迁移事务内的在飞门禁。
- **B167-A（控制面 API 域）** 相应**减少**一项：HTTP handler 不再自己做在飞检查，
  改为依赖 Store 的事务内门禁（handler 只负责把 Store 的错误翻成 409）。
- **B167-C（CLI 域）** 相应简化：不需要走 HTTP，也不需要在文档里声明能力边界——
  它和 HTTP 通道行为一致。上文 §2.4 结尾那个「1/2 二选一」的接缝**已消解**。
