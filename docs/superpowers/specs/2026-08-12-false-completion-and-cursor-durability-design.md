...
- B75：游标写不进去，`writeCursor` 打一条 Debug 日志就算了。审核者看到的是 `wait` 一遍遍返回旧事件，看不到「游标从来没写成功过」。
...
- `writeCursor` 失败只打 Debug；`readCursor` 失败（含权限错误、内容损坏）一律静默退回 0。
...
### 4.1 布局：`<游标根>/cursors/<target>/<taskID>`

`<target>` 取 `--target` 的值；直连本机（未指定 target）取字面量 `local`。
...
- **读失败分两类**：文件不存在 = 正常首次，退 0 且不报；其它错误（权限、内容损坏）**必须报到 stderr**，因为它们意味着「游标存在但用不了」，而后果是静默重放旧事件。
- **写失败**：按 §4.2 三级降级；降不动则响亮失败。任何情况下都不得只打 Debug 就返回。
...
- 命名空间：两个不同 target 的同 taskID 游标互不干扰。
...
| 6 | 多 target 同 taskID 不再串游标 | 命名空间测试 |
| 7 | 任务归档后游标被回收 | 回收三条测试 |
