...
状态：**已拍板（2026-09-16）**（handoff 派发形态——executor 出稿，本地协调者拍板；P-1..P-6 全选甲）
卡：B374
...
本稿在契约增量核对中发现 4 处**边界澄清**（C-1..C-4，已回写契约 §9，非自由偏好、不退回 contract），以及以下 P-1..P-6 真岔口。**2026-09-16 协调者全选甲**（用户授权自主推进，不再逐条问），裁决见 §9。
...
**2026-09-16 协调者裁决：P-1..P-6 全选甲。**用户授权「后面不要再问、自主推进到完成」。

| 编号 | 裁决 | 理由 |
| --- | --- | --- |
| P-1 | **甲** | 欠账 9 的语义本卡必须落；字面载体在本树只有 b156.2 两处「列表全量」。不从 `5318092f` 搬运 b358 文档（乙会引入无关文档冲突）。b358 回到本树后再补 §4.4。 |
| P-2 | **甲** | spec 只要排查定性。机内能证镜像发现与房间刷新无共享锁/goroutine，code-path 不是风暴次生。linux-01 真因进真机清单。不改 `discoverOnce` 预算。 |
| P-3 | **甲** | F9 必须在 HTTP handler 上有牙。13 处无参请求改 `limit=50` 保留原断言；另增无参 426 + 冻结文案。不绕到 service 直调。 |
| P-4 | **甲** | 契约 P1：调用方收窄入参即限域，刷新体不改。仍 `AllTaskLinks` 建索引，只按本页 rooms 取子集投影并传 `startRoomAttachRefresh`。不改 `d_ledger` 读面。 |
| P-5 | **甲** | `TestSetupWritesJSONToFile` 意图是 JSON 落盘，显式 `HANDOFF_LOG_LEVEL=info`。另增 `TestSetupDefaultLevelWarn` 锁 F19。禁止把 Info 改 Warn 迁就默认。 |
| P-6 | **甲** | F20 两半边都要锁：文件恰一次 **且** 不落 stderr。只测文件会给「把 stderr 挂回去」留假绿。 |

实现与 plan 以契约 §5 重编号为准，不以骨架注释里的旧 F 编号为准。`trimRoomPage` 注释「(LastActivity,roomID) 为兜底比较」已否决，implement 按契约 P6/F14。
