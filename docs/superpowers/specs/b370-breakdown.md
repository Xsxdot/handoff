...
状态：**已拍板**（2026-09-14；本稿由 handoff executor 出稿，本地协调者已裁决 F1–F7，裁决见 §9）
卡：B370
...
## 9. 拍板记录区（协调者回填，2026-09-14）

拍板人：本地协调者（用户在场；裁决范围限于 spec 已批准语义与契约已冻结面之内）。

| 编号 | 裁决 | 理由 |
| --- | --- | --- |
| **F1** | **A** | 空身份时以该事件所属卡上最新合格快照为「当前身份」，再比 source 列。只有 A 能同时满足「空身份应交付」与 `TestB2336StaleAttemptDoesNotWake` 保绿（spec §79 明写该回归不许松）；B 会让该回归转红。 |
| **F2** | **A** | 在 `internal/client` 包内新增未导出三态取数（有合格快照 / 有派发但无身份 / 无派发）：导出面不变、不新增跨域边，两个 reason 按契约区分；B 的两次全卡扫描会引入不一致。 |
| **F3** | **A** | 身份非空且缺 `source_task` 返回 `WakeGateMissingSourceTask`（枚举内，可观测性更细）。B349 的「缺 `source_task` 闭集拒绝叫醒」不动，只把 reason 精确化；本条同时消解 §0 列出的 §3.2 A 末句与 §3.1 注释不自洽。 |
| **F4** | **A** | `JudgeMirroredWake` 收到 `st == nil` 时显式返回带上下文错误，不 panic；两处消费点各自的错误路径保留。 |
| **F5** | **A** | 两个消费点各补一条按 `decision.Reason` 的日志（契约 §3.2 C 字面）；共享函数内的既有日志保留，不视为替代。 |
| **F6** | **A** | 单卡单轮（`B370-impl` 内部 T0–T4），不拆并行子卡——两处消费点承载同一条规则，拆卡会把规则拆散（同 B353 选档理由）。 |
| **F7** | **A** | 实现轮在自己的分支重扫并落 `codegraph/diffs/<implement-branch>.json`，使 `codegraph --view <branch> check` 成为对照判据；dead-contract 过渡态按 contract §8 已记的已知红处理。 |

拍板不改变契约 §3.2 判定正文与 §3.1 导出面，**无需重冻 contract**；§0 的两条 minor：后者（§3.2 A 末句不自洽）由 F3=A 消解，前者（未重扫前的 dead-contract 过渡态）归 F7 的实现轮重扫。
