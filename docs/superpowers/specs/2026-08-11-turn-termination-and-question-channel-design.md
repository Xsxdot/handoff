...
**e2e 回填（08-11）**：这条分支**没被走到**，因为触发不了。模型调 `question` 工具提出的那个问题，工单渲染里没有出现「可自定义（直接写答案）」标记——按 §4.2 的渲染规则，这等于 opencode 送来的 `QuestionInfo.custom` 是 false/缺省。也就是说，模型自己决定要不要开自定义答案，审核者这侧无法指定；本次它没开。

结论有两层：

1. **§2.4 那条推断仍未实证**。`custom: true` 时自由文本能否直接进 `answers`，真机没给出答案，因为真机根本没产出 `custom: true` 的请求。
2. **但这不改变处置**。正因为 `custom` 由模型侧决定、且默认不开，「阻塞探针」这条路本来就不可靠——它要等模型碰巧开一次自定义。§5.4 的运行时降级仍是正解：第 3 级发出去，4xx 就落回第 4 级重问。这条降级路径由 `ErrCustomAnswerRejected` 的单测覆盖，不依赖真机。
