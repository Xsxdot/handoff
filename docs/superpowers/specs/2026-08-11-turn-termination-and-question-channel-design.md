...
`custom: true` 时能否把自由文本直接放进 `answers` 透传，目前只有类型层面的推断（`QuestionAnswer = Array<string>`，是字符串数组而非枚举），没有运行时证据。处置见 §5.4。
...
§2.4 那条未实证的推断，作为实现计划的 **Task 0** 单独验：真机上构造一个 `custom: true` 的问题，用自由文本调 reply，看服务端是否接受。

- 成立 → §4.3 第 3 级按设计实现
- 不成立 → 该级降级为「直接走第 4 级重问」，其余设计不受影响

**先验再实现**，不要把未验证的分支写进代码再指望 e2e 兜住。
