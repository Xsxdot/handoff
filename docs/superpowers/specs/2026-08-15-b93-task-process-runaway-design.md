...
- **B92 的状态迁移缺陷**。两条都在 agentd 状态机附近，但根因不同，各自收口。
  注意：B92 若成立（`failed` 事件落库但状态没迁移），§3.1 挂在 `handleResult`
  上的清扫在那条路径上同样不会执行——**两条修复合起来才闭环**，实现时要在
  注释里点明这层依赖。
...
**与 B92 的耦合。** 见 §4 末条：B92 未修时，`failed` 事件落库但状态没迁移的那条
路径上，§3.1 的清扫同样不会执行。§3.2 的 watchdog 点名是这种情况下的兜底——它
按**状态**筛活跃任务，而卡住的任务状态恰恰是 `running`，会被扫到。**这是刻意的
冗余，不是重复。**
