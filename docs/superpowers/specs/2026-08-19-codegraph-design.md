...
- `domains`（2026-08-21 新增）：领域段，**扫描产出、人可在入库后改**。
  ...
  增加 `domain` 字段，归属到**叶子领域**。领域怎么切、切几层是扫描 AI 的产出物：
  **按职责/子系统切，不按包切**——顶层是 5~12 个子系统，实现它们的包与类作为子领域或
  容器挂下去。分层项目（controller/service/dao 各一个包）按包切会得到「层即领域」，
  那是架构模板不是这个系统的样子；同一件业务的各层属于同一个领域。切分细则见
  `docs/codegraph-scan-recipe.md` 的「怎么切领域」。原型阶段领域由容器包名推导 + 内置
  元信息演示，真实契约必须显式入库——领域间连线、对外接口表都从这份归属聚合而来。
