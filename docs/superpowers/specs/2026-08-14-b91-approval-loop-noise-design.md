...
### 3.5 路径域：纯路径请求也去重（08-14 实测后追加，已裁决做）
...
**裁决（08-14 用户定）：做，折进 B91。** `Perm.Command` 为空且 `Perm.Paths`
非空时走路径域：

```
sha256("paths\x00" + Tool + "\x00" + strings.Join(排序后的 Paths, "\x00"))
```

三个设计点，都不是可选项：

- **必须带 `Tool`**。路径域丢掉的不只是子 agent 前缀，还有 kind——`edit` 与
  `external_directory` 对同一路径含义不同（写这个文件 vs 授权越界访问这个
  目录），裸路径合并等于把两种授权当成一件事。带上 Tool 后，跨子 agent 与
  跨同 Tool 的重复都能治，而两种 Tool 之间仍然各批各的。
- **Paths 要排序**：同一组目录的顺序可能不稳，排序让集合相同即指纹相同；
  NUL 作分隔符（路径不可能含 NUL），杜绝 `["a","b/c"]` 与 `["a/b","c"]`
  这类拼接歧义。
- **三个域互不相撞**：`cmd\x00` / `paths\x00` 前缀各占一域，剩下的落全文域。

优先级：命令域 > 路径域 > 全文域。bash 形态的 `external_directory` 同时带
命令与目录，走命令域（既有 §7 风险条已覆盖）。
...
3. 单测：既无命令也无路径（`Perm` 为 nil 或两者皆空）的权限维持全文指纹行为
   （现有 B57 用例不红）
3b. 单测（§3.5）：同一路径集、同 Tool、带与不带 `[子 agent: …]` 前缀 → 复用
   命中零唤醒；同路径但 Tool 不同（`edit` vs `external_directory`）→ **不**复用；
   Paths 顺序颠倒 → 仍命中
