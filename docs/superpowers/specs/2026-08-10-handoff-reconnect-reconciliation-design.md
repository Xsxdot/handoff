...
- **六行判据回归（8/11，B38 Task9 修复后）**：dispatch 任务 `744d936f-3504-4502-8380-0131a55491a0`（plan2），20s kill agentd，黑洞窗口内轮询直至回合真正完结（`completed=1786390706461` 非零、提交 `d0a5de6` 落地），重启后 `对账完成：已补发断连期间丢失的终态 ... event=result armed=true` → `emitted=1` → `waiting_review`——**六行判据不砍掉 B38 已验能力**。
- **冻结尾部不补假终态（8/11，B38 Task9）**：派发工具密集任务后**同时杀 serve 与 agentd**（模拟机器重启，任务 `dfa2e4c1-491d-4d93-b796-be2ce51aea5b`；另一轮先杀 serve、agentd 存活，任务 `2fc17ee3-bd55-40c8-b2f0-efe975c0762a`）——会话冻结在纯工具消息尾部（`finish=tool-calls` + tool `status=completed`、completed 非零）。重启后**由运行时对账（executor 已不在）接管**：`executor 已不在，开始对账 ... from=running to=waiting_review`，任务带 failed 事件进 waiting_review，**会话对账未补任何假终态**。真机上会话对账未直接撞上冻结尾部（运行时对账先接管），该分支由确定性单测钉死（`TestReconcileSkipsFrozenToolTail`，真机报文夹具）。
