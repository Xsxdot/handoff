...
| # | 断言 |
|---|---|
| 1 | 回合已完结、水位落后 → 补发 1 条，水位前进 |
| 2 | 回合已完结、水位已过 → 补 **0** 条 ← 幂等的核心断言 |
| 3 | 会话仍在忙 → 补 0 条，不改状态 |
| 4 | 查询失败 → 补 0 条、返回 error，而 `Resume` 仍然成功 |
| 5 | **以提问收尾的回合被还原成 `question` 而不是 `result`** ← §3.3 的核心断言 |
| 6 | 悬而未决的权限被重新上报，manager 侧不出第二张工单 |
