...
**状态：已拍板（2026-09-09；协调者按用户「同意」落点，不改产品承诺）**
...
已拍板（2026-09-09）。正文按 P1-A 单卡布局；实现仍走 B353，不另 split。
...
1. **P1｜外部子卡形态。**
   ...
   - **拍板 A。** 轻档、B354/B355 已并入 B353，不 `card split`。内部 T0→T1→T2/T3→T4 走同一张实现卡；拆三张会把分类表拆散。
...
2. **P2｜操作文档同步面。**
   ...
   - **拍板 A。** skill 与 README 同批改；`card wait --follow` 是用户可见行为。文档绿不等于运行时已验。
...
3. **P3｜项目缺陷族清单的基线标记治理债。**
   ...
   - **拍板 B。** 本卡不改项目缺陷族清单顶部；缺标记入台账，不绑进唤醒面。
...
- [x] 待拍板岔口 P1–P3 集中在稿首；2026-09-09 已拍板 A/A/B，头部已改为已拍板。
