...
| P3 | Gatekeeper 不拦 `~/.local/bin` 里那份（它脱离了 `.app` 的签名覆盖） | macOS | §5 整条路走不通，须回到 bundle 内运行方案 | **✅ 已验 2026-08-19**（v0.3.0-rc10，取证见 plan 的 .notes.md）。关键事实：隔离属性传染到了 `.app`，但**没有**继续传染到它写出的文件上，于是 Gatekeeper 根本不评估它。同时了结 W5 spec §5.4 的 P2 |
...
P3 曾是最承重的一条：它若不成立，§5 整条路都要重做。**2026-08-19 已实测成立**，§5 的落点设计确认可行。

验它时暴露的两件事记在这里，因为它们会重复出现在后续走查里：

1. **`gh release download` 不设 `com.apple.quarantine`。** 不手工补上，整条验证是假绿——
   而 P3 问的恰恰是隔离属性会不会传染
2. **`stapler validate` 与 `spctl -t exec` 对裸可执行文件都会「失败」，但都不是缺陷。**
   苹果不支持给裸 Mach-O 装订票据；`spctl -t exec` 拒的理由是「这不是个 app」而非签名
   有问题（它同一行会写 `the code is valid`）。判据要先确认它适用于被验对象——这两条
   若当门用，会把一条成立的路径判成不成立
