...
| B15 | adapter.Start 失败泄漏 managed worktree | 💡 idea | 高 | — | — | — | — | 来源：08-09 三期压测实测——compensateManagedWorktree 只覆盖 MkdirAll/WriteFile/CreateTask 三条失败路径，未覆盖 ad.Start() 与其后 SetTaskField 的失败；实测 executor 起不来时任务落 failed 且 worktree_managed=true，而 done 要求 waiting_review，该任务永远归档不了 → worktree 与分支永久泄漏（实测残留 /Users/sycm/.handoff/worktrees/0995f42e）。修法：把补偿覆盖到 Dispatch 里 PrepareWorkspace 之后的**全部**失败返回点，或改 defer 统一补偿 |
