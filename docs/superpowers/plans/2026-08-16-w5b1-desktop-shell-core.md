...
审核者本地执行，2026-08-17，macOS 26.5.2 / M1 Pro。产物：合并后工作区
`wails3 task package` 出的 `handoff-desktop.app`（10.7MB）。

**说明一处与计划的偏差**：计划 Step 1 写的是用临时 datadir 起一个隔离 agentd。
实际走的是本机既有的 agentd（已运行 25h39m，launchd 托管）。原因是薄壳读的是
`config.DefaultPath()`（`~/.handoff/config.yaml`），要让它连临时 agentd 就得改 HOME，
而那样 `service.Manager` 又会去动 launchd 单元——为了隔离而引入的动作比被测行为本身更危险。
代价是走查会在本机 agentd 上留一个真实会话，**事后已吊销**（见下）。

#### 已验证（有实际输出）

| # | 项 | 实际证据 |
|---|---|---|
| 1 | 构建与打包 | `wails3 task package` rc=0，产出 `bin/handoff-desktop.app`（10,698,962 字节） |
| 2 | 启动 | 双击（`open -n`）后进程起来，pid=85995 |
| 3 | 窗口 | `CGWindowList` 查到该 pid 的窗口 `1175x815 @ (440,277)` |
| 4 | **握手 + 控制台真的加载了** | 会话数 **7 → 8**，新增 `handoff-desktop (MacBook-Pro.local)`。这条是间接但充分的证据：只有 webview 真的打开了 `/console?ticket=…` 并把 ticket 换成 cookie，agentd 侧才会多出这条会话。设备名与 `DefaultDeviceName()` 的规格逐字一致 |
| 5 | **P4 机制层面** | `agentd pid=36100`，**父进程 pid=1（launchd）**；薄壳 pid=85995。薄壳不在 agentd 的进程树里，因此不可能通过进程组把它带走 |
| 6 | **P4 行为层面（部分）** | 直接 `pkill` 掉薄壳进程后，`handoff status` 仍返回「agentd 可用」。杀进程比关窗口更狠，这条能覆盖「薄壳消失不影响 agentd」 |
| 7 | 清理 | 走查建立的会话已 `DELETE /api/auth/sessions/<id>` 吊销，HTTP 200 |

#### 未验证（不许当成通过）

| 项 | 卡在哪 |
|---|---|
| 托盘图标的外观、菜单两项的文案与可点击性 | 本机未授予**辅助访问**，`osascript` 枚举菜单栏返回 `-1719 不允许辅助访问`；也未授予屏幕录制，截不了图 |
| 关闭窗口（而非杀进程）后进程与托盘是否仍在 | 同上，无法用脚本点关闭按钮 |
| 目录选择器的实际交互 | 同上，需要人点 |
| **P4 端到端**：任务真的在跑时关掉薄壳，执行者是否存活 | 需要在本机跑一个真实任务。已验的第 5、6 条覆盖了机制与进程层面，但**没有覆盖「有活跃执行者时」这一场景** |

补齐这四项只需要用户在本机授予辅助访问/屏幕录制，或者手动看一眼——不需要改代码。
在补齐之前，**不要把 spec §9「交付物②」的验收标准标成已通过**。
