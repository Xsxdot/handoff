# B231-2 一次性扫描 plan：handoff 全量重扫（补职责面首扫）

本次任务参数：
- 项目：handoff（本仓）
- 形态：**全量重扫**，只更新 codegraph/baseline.json；meta.branch/commit 写当前分支与 HEAD
- 本轮特别要求（B231 拆解稿裁决，优先级高于下文正文的冲突条款）：
  1. **packages 节首扫**：对每个出现在节点 file 上的目录建条目，Go 包转录 doc 注释、无注释空串、TS/React 目录条目 summary 固定空串——按下文 Schema 与「摘要抓取红线」执行；
  2. **类型/函数 doc 必抓**：有 doc 注释的 model 与 func 节点 summary 100% 收录；
  3. **card 族追链到位（岔口二裁决）**：`handoff card` 全族 24 个子命令的 entry 及其调用链必须扫全，**本轮禁用「标 unscanned: true 兜底」**（下文第 50 行附近的宁缺毋滥条款对 card 族不适用）；其他入口实在追不动的，逐条列入交付说明（这些方向将不参与预算重定标）；
  4. **文件级完整性自检**（正文已有，必做）：盘上文件数=图中文件数，差集逐个说明；重点核 cmd/（上轮实测 9 文件零节点）；
  5. **禁动清单**：codegraph/target.json、codegraph/domains/*.json、codegraph/best.json 一律只读；
  6. 交付说明附统计：packages 条目数 vs 图中目录数；model/func 各自「有 doc 注释数 / 已收录数」；unscanned 残留清单（应为空或逐条说明）。

以下为扫描配方正文（docs/codegraph-scan-recipe.md @ main 2fdfeff1a，逐字执行）：
