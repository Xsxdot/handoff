...
| 3 | 列表 `GET /api/sessions` 信封 `{"sessions":[SessionSummary…]}`，member 维度=服务端注入 `web:<host>`，`?member=` 不参与；未读=该 member 游标投影（**列全部会话，不按成员过滤**） | `sessionsapi.go#handleSessionsList`（:94-104）、`internal/collab/sessions.go#ListSessions`（:30-64） |
| 4 | 详情 `GET /api/sessions/{id}` → `SessionDetail` 全文；不存在 404 且文案含会话 id | `sessionsapi.go#handleSessionDetail`（:111-121） |
...
| 8 | `RoomMessage.ReplyTo int64 omitempty`（`reply_to`）：零值省键、非零在线、roundtrip 恒等 | `internal/proto/rooms.go`；`sessions_fixture_test.go#TestRoomMessageReplyToGolden` |
...
| 10 | 真仓 tabbar 词汇：**tab=组**（TabBar.tsx 头注释），组标签=焦点内容名（autoName）；会话行开 tab=`openOrFocus`（全局搜 dedupKey，不命中开新组）→ tabbar 每会话一项；关 = 组关闭；◫ = place 到焦点格右列 | `web/src/app/workbench/TabBar.tsx:1-20`、`tabs.ts#openOrFocus`（:262-275）、`#placeSource` zone:'right'（:473-477） |
