...
### ⚠ 不要调 `MaybeInstallService`，也不要给 `InstallService` 赋值（2026-08-17 派发中补）
...
的 `init()` 注入，并在 `nil` 时兜底不 panic。CLI 侧因此照常工作。

薄壳不 import `cmd`（那正是 §4.4.2 的全部理由），所以桌面路径上这个变量恒为 `nil`。
**但这无害，因为薄壳根本走不到它**：`AskAll` 自己不调 `MaybeInstallService`，
后者是 `cmd/init.go:135` 在 `AskAll` **之后**单独调的。装配序列只要不主动调它就没事。

> 一度以为这里有缺陷、要把缝接到薄壳侧——那是错的，读 `lifecycle.go` 后纠正。
> **`shell.EnsureRunning` 已经做完了全部托管**：拿 manager → 查状态 → 没在跑就
> `m.Install(spec)`（装单元并拉起）。再叠一层 `MaybeInstallService` 是重复。

**装配序列保持不变**：向导 → `config.Save` → `EnsureRunning` → `ConsoleURL` → `SetURL`。

#### 与 spec §4.4 的一处有意偏离（必须记进 ledger）

spec §4.4 把「是否装 service」列进了向导要覆盖的决策。**桌面这条路上不问**：

CLI 里答「否」是合理的——用户可以自己跑 `handoff agentd`，或稍后 `handoff service install`。
但薄壳答「否」意味着**没有 agentd 可连**，窗口打不开控制台，应用当场没用。
**一个「否」会让程序不能工作的问题，不该问。** 桌面上托管不是可选项，是前置条件。

ledger 里记「与 spec §4.4 的有意偏离 + 上述理由」，别让它看起来像漏做了。
