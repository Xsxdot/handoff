...
**L3轻**。定级两问：① 改动跨 `d_gateway`（handler+刷新）/`d_sessions`（collab 列表语义）/`d_web`（懒加载）三个子系统契约面；② 动跨进程 wire 契约 `GET /api/rooms`（外置桌面 app 同进程外消费），即便加参相容、强制分页也是行为 breaking。选档：单域切片均未超流程固定成本（约 70 分钟），→ 轻档。轻档不跳契约冻结：分页 wire 语义由 contract 节点冻结。
