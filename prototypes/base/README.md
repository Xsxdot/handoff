...
| pages/settings.html | 设置页（开发机/执行纪律/常规/Env/**更新**） | `/settings` | 确认中 |
| （未建页） | 任务深链：按任务 id 直接打开它的 TUI | `/tasks/:id` | — |
| （无需页） | `/machines` → 重定向到 `/settings` | `/machines` | — |
| （未建页） | 工作项账本看板（统一任务卡：看板/列表双视图、「需要你」就地筛选、详情抽屉含并入区/验收/Timeline、合并跟随可拆回） | 规划中（workbench 一期，spec `docs/superpowers/specs/2026-08-18-workbench-phase1-design.md`） | 确认中 |
| （未建页） | 流程管理页（工作流状态形状 + gate、派发模板 + 纪律块版本，均不可变版本化） | 规划中（同上） | 确认中 |
...
默认沉默、仅异常时上浮。真实页面开发完成后对照该副本验收，通过后回流并推进为已确认。
...
更新形态（B166）经 fork 副本 `desktop-update/` 走查确认（2026-08-19），**该副本即验收基准**：
① 控制台右下角提示框接管原来挂在菜单栏图标上的三条动态提示（有新版 / 有更新待应用 /
上次同步失败），主按钮就地转进度条、完成后文案变「正在重启…」，与 home 悬浮窗共用右下角
时整摞上移让位；② 设置页新增「更新」分区，三块为桌面应用自更新 / 同步状态（提示框消失后
的常驻落点）/ 执行机版本与一键升级，本机那行标「随桌面应用一起更新」且不给按钮；
③ 托盘只剩「打开控制台」与「退出（agentd 继续运行）」，图标换成标志。设计见 spec
